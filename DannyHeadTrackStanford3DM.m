...
cd('H:\New Danny\20170113\');
[filename,pathname] = uigetfile({'*.*'},'Pick Text File to Analyze'); %change file name
full = fullfile(pathname,filename) ;
databdr = imread(full,1);
...
[filename,pathname] = uigetfile({'*.*'},'Pick Text File to Analyze'); %change file name
full = fullfile(pathname,filename) ;
file = full; %name of file to be read
...
for i = 1:nver-1
    slp1 = (ybd1(i+1)-ybd1(i))/(xbd(i+1)-xbd(i)); %slope
...
%% section to calculate change in angle of incidence
    ...
    for z = 1:npd
        ...
        if maxbd == 5 && locbdr(1) == 1 %test that it crossed region and started inside
            ...
            if loc_crs > chklen % can we track it for 10 frames before crossing?
                %disp('accept');
                entrydatax = traj(locz(loc_crs-chklen+1):locz(loc_crs-1),2); % take entry data
                entrydatay = traj(locz(loc_crs-chklen+1):locz(loc_crs-1),1);
            elseif loc_crs >= 5 && loc_crs <= 10 % if we can track it for more than 5 frames before and less than 10
                entrydatax = traj(locz(1):locz(loc_crs-1),2); % take entry data
            ...
            if remlen > chklen
                exitdatax = traj(locz(loc_crs):locz(loc_crs+chklen-1),2); % take exit data
                exitdatay = traj(locz(loc_crs):locz(loc_crs+chklen-1),1);
            elseif remlen >= 5 && remlen <= 10
                exitdatax = traj(locz(loc_crs):locz(end),2); % take exit data
                exitdatay = traj(locz(loc_crs):locz(end),1);
            else
                exitdatax = [];
                exitdatay = [];
            end
            entrydatay1 = abs(512 - entrydatay);
            exitdatay1 = abs(512 - exitdatay);
            if ~isempty(entrydatax) && ~isempty(exitdatax)% if we have enough tracks before and after passing over border
            fitin = polyfit(entrydatax,entrydatay,1); %fit entry data with line [m b] so first is the slope then constant
            fitin1 = polyfit(entrydatax,entrydatay1,1);
            fitex = polyfit(exitdatax,exitdatay,1); %fit exit data with line
            fitex1 = polyfit(exitdatax,exitdatay1,1);
            lfangin = atan(fitin(1));
            lfangex = atan(fitex(1));
            %this portion should check the location of angle of incidence
            boxnum = 0;
            for i = 1:3 % consider the first 3 boundaries only
                ...
                if xrc >= xsml && xrc <= xbig && yrc >= ysml && yrc <= ybig % defines the lines we want to consider for each boundary
                    boxnum = i;
                    %disp(boxnum);
                end
            end
            if boxnum ~= 0 % if there are any particlles which crossed one of our boundaries
                disp(z); % display the particle number which crossed
                ...
                thintan = acosd(dot(vectan,vecinlf)); %entry angle from tangent
                thoutan = acosd(dot(vectan,vecexlf)); %exit angle from tangent
                disp(thin);
                disp(thou);
                angle(ctr,1) = atan2(fitin1(1), 1) - atan2(normdat(boxnum,5), 1);
                if (angle(ctr,1) < 0) 
                    angle(ctr,1) = angle(ctr,1) + 2*pi;
                end
                angle(ctr,2) = atan2(fitex1(1), 1) - atan2(normdat(boxnum,5), 1);
                if (angle(ctr,2) < 0) 
                    angle(ctr,2) = angle(ctr,2) + 2*pi;
                end
...
%                     angres(ctr,2) = (2*pi-thou)*180/pi;
%                 end
%                 angres(:,1) = angres(:,1)*180/pi;
%                 angres(:,2) = angres(:,2)*180/pi;
%                 if thintan < pi/2 && th
...
figure; 
angle1 = 180.*angle./pi;
scatter(angle1(:,1),angle1(:,2));

%% display party
disp('PARTY.....');
disp('Calculating frequency is so easy it hertz...');