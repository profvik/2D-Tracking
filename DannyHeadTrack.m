...
file = 'H:\New Danny\20170113\20170113_ch05_MyLOVChar_EX4R_1-4dilution_2mMATP_2mMMag_1mspf_600frames_150Xmag 01.tif'; %name of file to be read
...
for i = fini:diff:fend
    ...
    level = 0.50;%graythresh(imini); %find thresholding level
...
% %figure;
% %hold;
% z = 10%loop over number of particles
...
% quiver(tempdat(1:end-1,1),tempdat(1:end-1,2),uv(:,1),uv(:,2),0.25);
 ...
 %% velocity distribution section
    ...
    for z = 1:npd
        ...
        locz = find(traj(:,4) == z);
        tempdat = traj(locz,:);
        locino = find(tempdat(:,5) == 1); %this portion removes particle
        tempdat(locino,:) = [];
        npid = size(tempdat);
        xstd = std(tempdat(:,1));
        ystd = std(tempdat(:,2));
...
%% section to calculate change in angle of incidence
    ...
    for z = 1:npd
        ...
        if maxbd ~= minbd && locbdr(1) == 1 %test that it crossed region and started inside
            ...
            if loc_crs > chklen
                entrydatax = traj(locz(loc_crs-chklen+1):locz(loc_crs-1),1);
                entrydatay = traj(locz(loc_crs-chklen+1):locz(loc_crs-1),2);
            elseif loc_crs > 3 && loc_crs <= 10
                entrydatax = traj(locz(1):locz(loc_crs-1),1);
                entrydatay = traj(locz(1):locz(loc_crs-1),2);
            else
                entrydatax = [];
                entrydatay = [];
            end
            ...
            if remlen > chklen
                exitdatax = traj(locz(loc_crs):locz(loc_crs+chklen-1),1);
                exitdatay = traj(locz(loc_crs):locz(loc_crs+chklen-1),2);
            elseif remlen > 3 && remlen <= 10
                exitdatax = traj(locz(loc_crs):locz(end),1);
                exitdatay = traj(locz(loc_crs):locz(end),2);
            else
                exitdatax = [];
                exitdatay = [];
            end
            ...
            boxnum = 0;
            for i = 1:3
                xrc = traj(locz(loc_crs),1); %x position of region change
                yrc = traj(locz(loc_crs),2); %y position of region change
            ...
            if boxnum ~= 0
                ...
                thin = acos(dot(vecnorm,vecinlf));
                thou = acos(dot(vecnorm,vecexlf));
